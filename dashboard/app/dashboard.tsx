...
type DashboardData = {
  metadata: { symbol: string; display_name: string; strategy: string; interval: string; date_range: string; is_synthetic: boolean };
  config: { initial_capital: number; quantity: number; opening_range_minutes: number; stop_loss_pct: number; take_profit_pct: number; force_exit_time: string; commission_rate: number; sell_tax_rate: number; slippage_bps: number };
...
function CandleChart({ bars, trade, config }: { bars: Bar[]; trade: Trade; config: DashboardData["config"] }) {
  ...
  return <div className="chart-box">
    ...
    <svg viewBox={`0 0 ${W} ${H}`} role="img" aria-label="分鐘 K 線與進出場位置">
      ...
      <rect x={left} y={top} width={config.opening_range_minutes / Math.max(bars.length - 1, 1) * pw} height={ph} className="opening-zone" />
...
export default function Dashboard() {
  ...
  const s=data.summary,t=data.trades[selected],day=t.entry_time.slice(0,10),bars=data.bars.filter(b=>b.timestamp.startsWith(day));
  const wins=data.trades.filter(x=>x.net_pnl>0).length;

  return <main className="shell">
    <header className="topbar"><div className="brand"><b>WQ</b><div><span>WADE QUANT LAB · BACKTEST 01</span><h1>台股當沖策略儀表板</h1></div></div><div className="headmeta"><em>● 回測完成</em><span>{data.metadata.date_range}</span></div></header>
    <section className="instrument"><div><strong>{data.metadata.symbol}</strong><span>{data.metadata.display_name}</span></div><div className="tags"><span>{data.metadata.strategy}</span><span>{data.metadata.interval}</span><span>每筆 {money.format(data.config.quantity)} 股</span><span className="synthetic">合成資料 · 非投資建議</span></div></section>
    <section className="metrics">
      <Metric label="淨利" value={signedMoney(Number(s.net_profit))} note={`交易成本 NT$ ${money.format(Number(s.total_cost))}`} tone="positive"/>
      <Metric label="總報酬" value={signedPct(Number(s.return_pct))} note={`期末資產 NT$ ${money.format(Number(s.ending_equity))}`} tone="positive"/>
      ...
      <Metric label="Profit Factor" value={decimal.format(Number(s.profit_factor))} note="總獲利 ÷ 總虧損"/>
      <Metric label="日頻 Sharpe" value={decimal.format(Number(s.daily_sharpe))} note="樣本僅 10 個交易日" tone="warning"/>
    </section>
    ...
    <div className="analysis">
      ...
      <aside className="panel risk-panel"><span className="kicker">RISK CHECK</span><h2>承擔風險</h2><div className="risk-score"><div className="risk-ring"><b>{decimal.format(Number(s.max_drawdown_pct))}%</b><small>MAX DD</small></div><div><strong>短樣本風險可能被低估</strong><p>目前僅 10 個交易日，尚未覆蓋盤整、跳空與極端行情。</p></div></div>
        ...
        <div className="alert"><b>主要警訊</b><p>交易成本 NT$ {money.format(Number(s.total_cost))}，是淨利的 {decimal.format(risk.costToNet)} 倍。實盤前必須驗證券商折扣、滑價與成交量限制。</p></div>
      </aside>
    </div>
    ...
    <footer><p>合成分鐘資料僅用於驗證回測與視覺化流程，不代表真實市場績效，也不構成投資建議。</p><p>開盤區間 {data.config.opening_range_minutes} 分鐘 · 停損 {data.config.stop_loss_pct*100}% · 停利 {data.config.take_profit_pct*100}% · {data.config.force_exit_time} 平倉</p></footer>
  </main>;
}

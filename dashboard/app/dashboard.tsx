...
export default function Dashboard() {
  ...
  const [options,setOptions]=useState<BacktestOptions|null>(null),[strategy,setStrategy]=useState("orb"),[interval,setInterval]=useState<Timeframe>("1m"),[start,setStart]=useState(""),[end,setEnd]=useState("");
  const runBacktest=async(nextStrategy=strategy,nextStart=start,nextEnd=end,nextInterval=interval)=>{
  ...
  return <main className="shell">
    ...
    <section className="backtest-controls"><label><span>交易策略</span><select value={strategy} onChange={e=>setStrategy(e.target.value)}>{options?.strategies.map(item=><option key={item.key} value={item.key}>{item.kind==="composite"?`組合 · ${item.name}`:item.name}</option>)}</select></label><label><span>K 棒週期</span><select value={interval} disabled={strategy.startsWith("composite:")} onChange={e=>setInterval(e.target.value as Timeframe)}>{strategy.startsWith("composite:")&&<option value={interval}>由組合策略決定</option>}{!strategy.startsWith("composite:")&&options?.intervals.map(item=><option key={item.key} value={item.key}>{item.name}</option>)}</select></label><label><span>開始日期</span><input type="date" value={start} min={options?.available_start??undefined} max={end||(options?.available_end??undefined)} onChange={e=>setStart(e.target.value)}/></label><label><span>結束日期</span><input type="date" value={end} min={start||(options?.available_start??undefined)} max={maximumEnd} onChange={e=>setEnd(e.target.value)}/></label><button disabled={loading} onClick={()=>runBacktest()}>{loading?"執行中…":"執行回測"}</button><small>由 1 分 K 逐根驅動 · 單次最多 {options?.max_days??31} 天</small></section>

...
export default function Dashboard() {
  ...
  if(!data||!risk)return <main className="shell"><header className="topbar"><div className="brand"><b>WQ</b><div><span>WADE QUANT LAB · BACKTEST 03</span><h1>微型臺指期貨策略回測</h1></div></div><div className="headmeta"><em>● {loading?"回測中":"等待執行"}</em><span>{start&&end?`${start} ～ ${end}`:"選擇回測期間"}</span></div></header><SystemNav active="/backtest/" />{controls}{error&&<div className="live-error">{error}</div>}<section className="panel empty-result"><strong>{loading?"正在執行回測…":"選擇條件後執行回測"}</strong><p>{loading?"完成後會在此顯示績效、圖表與交易明細。":"開啟頁面不會自動執行，避免每次進入都重新掃描歷史 K 棒。"}</p></section></main>;

...
export default function CompositeBuilder({ strategies }: { strategies: AtomicStrategy[] }) {
  ...
  const load = async () => {
    ...
    setTemplate(body.template); setSaved(body.strategies); setArchivedSaved(body.archived_strategies ?? []); setReferenceOptions(body.reference_strategies ?? []);
  ...
  return <section className="composite-area">
    ...
    <div className="composite-layout">
      ...
      <div className="panel composite-editor">
        ...
        <RuleGroupEditor title="1 · SETUP" hint="高週期背景／啟動條件，可留空" value={draft.setup} strategies={strategies} compositeOptions={referenceOptions} excludedCompositeId={editingId} onChange={value => setGroup("setup", value)} />
        <RuleGroupEditor title="2 · ENTRY" hint="真正觸發進場，至少一條" value={draft.entry} strategies={strategies} compositeOptions={referenceOptions} excludedCompositeId={editingId} required onChange={value => setGroup("entry", value)} />
        <RuleGroupEditor title="3 · EXIT" hint="策略出場條件；ANY 適合先到先出" value={draft.exit} strategies={strategies} compositeOptions={referenceOptions} excludedCompositeId={editingId} onChange={value => setGroup("exit", value)} />
